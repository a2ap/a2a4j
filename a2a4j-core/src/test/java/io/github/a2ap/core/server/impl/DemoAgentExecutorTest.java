...
class DemoAgentExecutorTest {
	...
	void setUp() {
		...
		Message userMessage = Message.builder()
			...
			.role("user")
			.parts(List.of(TextPart.builder().text("请帮我生成一个简单的Java类").build()))
			.build();

		taskContext = RequestContext.builder().task(task).build();
	...
	void testExecute_ShouldEmitMultipleEvents() {
		...
		StepVerifier.create(eventFlux).expectNextMatches(event -> {
			// 第一个事件应该是工作状态
			...
			.expectNextMatches(event -> {
				...
				if (event instanceof TaskStatusUpdateEvent) {
					TaskStatusUpdateEvent statusEvent = (TaskStatusUpdateEvent) event;
					return statusEvent.getStatus().getState() == TaskState.COMPLETED && statusEvent.getIsFinal();
				}
				return false;
			})
			.expectComplete()
	...
	void testExecute_ShouldEmitArtifacts() {
		...
		// 验证产物事件
		List<SendStreamingMessageResponse> events = eventQueue.asFlux().collectList().block(Duration.ofSeconds(10));

		assertThat(events).isNotNull();

		// 检查是否包含产物事件
		long artifactEventCount = events.stream().filter(event -> event instanceof TaskArtifactUpdateEvent).count();
	...
	void testExecute_ShouldCloseQueueWhenComplete() {
		// 执行代理任务
		agentExecutor.execute(taskContext, eventQueue).block(Duration.ofSeconds(10));
